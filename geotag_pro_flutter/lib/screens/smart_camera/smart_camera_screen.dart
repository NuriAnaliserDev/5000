...
class SmartCameraScreen extends StatefulWidget {
  final int? stationId; // if set, append photo to existing station
...
class _SmartCameraScreenState extends State<SmartCameraScreen> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  // Centralized Theme Getters (Enforced high-contrast for field use)
  ...
  void _checkShowTutorial() {
    final settings = context.read<SettingsController>();
    if (settings.hasSeenCameraTutorial) return;
  ...
  // Dynamic sensitivity for horizon responsiveness.
    ...
    _compassSub = FlutterCompass.events?.listen((event) {
      ...
      if (h != null && !h.isNaN) {
        ...
        if (_headingDeg == null) {
          ...
          double diff = target - _headingDeg!;
  ...
  void _recomputeOrientation() {
    ...
    if (now.difference(_lastSensorUpdate).inMilliseconds < delay) return;
    ...
    double targetAz = result.azimuth;
    if (_headingDeg != null) {
      double hd = _headingDeg! - targetAz;
      if (hd > 180) hd -= 360;
      if (hd < -180) hd += 360;
      if (hd.abs() < 35) {
    ...
    double azDiff = targetAz - _azimuth;
    ...
    double stDiff = result.strike - _strike;
    if (stDiff > 180) stDiff -= 360;
    if (stDiff < -180) stDiff += 360;
