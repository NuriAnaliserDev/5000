...
class LithologyNormalizer {
  ...
  static AIAnalysisResult normalize({
    ...
    // 1. Basic Parse
    String rawRockType =
        (parsedJson['rockType'] ?? 'Unknown').toString().trim();
    List<String> rawMinerals =
        List<String>.from(parsedJson['mineralogy'] ?? []);
    ...
    List<RockType> parsedRockCandidates =
        _parseRockCandidates(cleanedRockString);
    ...
    if (parsedRockCandidates.length > 1) {
      trustReasons
          .add("Natijada bir nechta tosh turlari ko'rsatilgan (noaniqlik).");
      finalScore *= 1 - (parsedRockCandidates.length * 0.1);
    }

    if (imageQualityScore < 0.5) {
      trustReasons
          .add("Rasm sifati pastligi tahlilga salbiy ta'sir ko'rsatdi.");
    }
    ...
    if (validation.status == ValidationStatus.invalid || finalScore < 0.3) {
      relLevel = 'reject';
      trustReasons.add(
          "Geologik mantiqqa to'g'ri kelmaydi yoki ishonchlilik o'ta past.");
    } else if (finalScore < 0.7) {
      relLevel = 'low';
      trustReasons.add(
          "Tizim ishonchliligi past, mutaxassis tekshiruvi talab etiladi.");
    ...
    if (relLevel == 'high') {
      trustReasons.add(
          "Tahlil geologik mantiq va sifat tekshiruvlaridan muvaffaqiyatli o'tdi.");
    }
    ...
    String status =
        _determineStatus(validation.status, finalScore, allWarnings);
  ...
  static String _cleanupRockType(String type) {
    ...
    String t = type
        .replaceAll(RegExp(r'rock', caseSensitive: false), '')
  ...
  static List<Mineral> _parseMinerals(List<String> rawMinerals) {
    ...
    return rawMinerals
        .map((m) {
          ...
          if (goldAliases.any((a) => lower.contains(a))) {
            norm = 'Gold';
          } else if (quartzAliases.any((a) => lower.contains(a)))
            norm = 'Quartz';

          final percentMatch =
              RegExp(r'(\d+)\s*(?:%|percent|foiz)').firstMatch(lower);
          double? pct;
        ...
        .where((m) =>
            m.rawName.isNotEmpty &&
            m.rawName.toLowerCase() != "noma'lum" &&
            m.rawName.toLowerCase() != 'unknown')
        .toList();
  }
}
