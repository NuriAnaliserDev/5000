...
class ProjectedTrace {
  ...
  ProjectedTrace(
      {required this.points, required this.depth, required this.stationId});
}
...
class GeologicalProjectionService {
  ...
  static List<LatLng> projectStation(Station station, double depthMeters) {
    ...
    final shiftedCenter =
        _movePoint(LatLng(station.lat, station.lng), dipDir, horizontalShift);
  ...
  static LatLng _movePoint(
      LatLng start, double azimuthDeg, double distanceMeters) {
