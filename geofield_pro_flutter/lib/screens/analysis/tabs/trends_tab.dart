...
class _TrendsTabState extends State<TrendsTab> {
  ...
  void _compute(List<Station> stations) {
    ...
    _avgStrike = axialStrikes.isNotEmpty
        ...
        : (unimodalStrikes.isNotEmpty
            ? GeologyUtils.circularMean(unimodalStrikes)
            : 0.0);
        ...
        Container(
          ...
          child: Row(
            ...
            children: List.generate(9, (i) {
              final h = (_maxDipCount > 0)
                  ? (_dipCounts[i] / _maxDipCount) * 150
                  : 0.0;
              return Expanded(
                child: Column(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _insightRow(context.loc('trend_density'),
              _stats.kappa.isNaN ? '—' : _stats.kappa.toStringAsFixed(1)),
          _insightRow(
              context.loc('trend_reliability'),
              _stats.alpha95.isNaN
                  ? '—'
                  : '±${_stats.alpha95.toStringAsFixed(1)}°'),
        ],
      ),
    );
  }
