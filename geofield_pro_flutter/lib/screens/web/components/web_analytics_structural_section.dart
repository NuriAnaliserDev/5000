...
class WebAnalyticsStructuralSection extends StatefulWidget {
  ...
  @override
  State<WebAnalyticsStructuralSection> createState() =>
      _WebAnalyticsStructuralSectionState();
}

class _WebAnalyticsStructuralSectionState
    extends State<WebAnalyticsStructuralSection> {
  double _rotX = 0.5;
  double _rotY = 0.5;
        ...
        children: [
          Row(
            children: [
              ...
              const Text("Strukturaviy Trendlar Tahlili (Global)",
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const Spacer(),
              Text("${widget.stations.length} ta o'lchov nuqtasi",
          ...
          Row(
            children: [
              _buildStatBox("O'rtacha Strike",
                  "${avgStrike.toStringAsFixed(1)}°", Colors.blue),
              _buildStatBox(
                  "Ishonchlilik (α₉₅)",
                  "±${stats.alpha95.toStringAsFixed(1)}°",
                  stats.isReliable ? Colors.green : Colors.orange),
              _buildStatBox("Dispersion (κ)", stats.kappa.toStringAsFixed(1),
                  Colors.purple),
              _buildStatBox("Resultant (R)",
                  stats.resultantLength.toStringAsFixed(2), Colors.teal),
            ],
          ),
          const SizedBox(height: 24),
          const Text("3D STRUKTURAVIY MODEL",
              style: TextStyle(
                  fontSize: 14,
