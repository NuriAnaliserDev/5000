      ...
      polylines: [
        ...geoLines.where((l) => !l.isClosed).expand((l) {
          Color color = l.colorHex != null
              ...
              : Color(int.parse(GeologicalLine.defaultColorHex(l.lineType),
                      radix: 16) +
                  0xFF000000);
        ...
        if (isDrawingMode && drawingPoints.isNotEmpty)
          Polyline(
            points: selectedLineType == 'polygon'
                ? [
                    ...(isCurvedMode
                        ? LineworkUtils.smoothLine(drawingPoints)
                        : drawingPoints),
                    (isCurvedMode
                            ? LineworkUtils.smoothLine(drawingPoints)
                            : drawingPoints)
            ...
            color: Color(int.parse(
                    GeologicalLine.defaultColorHex(selectedLineType),
                    radix: 16) +
                0xFF000000),
            strokeWidth: 3.0,
            pattern: selectedLineType == 'fault'
                ? StrokePattern.dashed(segments: const [10, 5])
                : const StrokePattern.solid(),
          ),
      ],
