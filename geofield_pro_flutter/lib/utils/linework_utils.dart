...
class LineworkUtils {
  ...
  static LatLng _closestOnSegment(LatLng p, LatLng a, LatLng b) {
    ...
    var t = ((p.latitude - a.latitude) * dLat +
            (p.longitude - a.longitude) * dLng) /
        l2;
    t = t.clamp(0.0, 1.0);
  ...
  static double minDistanceMetersToLine(LatLng tap, GeologicalLine l) {
    ...
    final raw =
        List.generate(l.lats.length, (i) => LatLng(l.lats[i], l.lngs[i]));
  ...
  static LatLng _catmullRom(
      LatLng p0, LatLng p1, LatLng p2, LatLng p3, double t) {
    final double t2 = t * t;
  ...
  static List<List<LatLng>> calculateThrustTeeth(List<LatLng> points,
  ...
  static LatLng? findNearestSnapPoint(LatLng tapPoint, List<LatLng> candidates,
      {double thresholdDegrees = 0.0005}) {
    LatLng? nearest;
  ...
  static LatLng nearestGridPoint(LatLng p, double gridMeters) {
    ...
    final gx = (p.longitude * mPerDegLng / gridMeters).round() *
        (gridMeters / mPerDegLng);
