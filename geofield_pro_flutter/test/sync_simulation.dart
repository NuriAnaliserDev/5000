...
import '../lib/models/geological_line.dart';
import '../lib/models/map_structure_annotation.dart';
import '../lib/models/sync_item.dart';
import '../lib/utils/sync_utils.dart';
import '../lib/services/sync/sync_queue_service.dart';
import '../lib/services/station_repository.dart';
import '../lib/services/cloud_sync_service.dart';
import '../lib/services/sync/conflict_queue_service.dart';
import '../lib/services/sync/sync_processor.dart';
import '../lib/models/sync_conflict.dart';
import '../lib/services/hive_db.dart';
...
void main() {
  ...
  group('Sync Reliability Simulation', () {
    ...
    setUp(() async {
      ...
      stationRepository = StationRepository(MockCloudSyncService(), queueService);
    });
    ...
    test('Offline Batch & Dedup: Coalescing multiple operations', () async {
      ...
      for (int i = 0; i < 50; i++) {
        ...
        for (int j = 0; j < 5; j++) {
          await stationRepository.updateStation(key, s.copyWith(name: 'Station $i Update $j'));
        }
      }
      ...
      final pending = syncBox.values.where((item) => item.status == SyncStatus.pending).toList();
    ...
    test('Crash Recovery: Restoring stuck processing items', () async {
      ...
      expect(syncBox.values.where((item) => item.status == SyncStatus.processing).length, 5);
      
      await queueService.resetProcessingItems();
      
      expect(syncBox.values.where((item) => item.status == SyncStatus.processing).length, 0);
      expect(syncBox.values.where((item) => item.status == SyncStatus.pending).length, 5);
    });

    test('Conflict Resolution: Deterministic tie-breaker', () {
      // 1. Remote higher version wins
      expect(SyncUtils.shouldRemoteOverwriteLocal(
        localVersion: 5, localUpdatedAt: DateTime(2026, 1, 1), localDeviceId: 'A',
        remoteVersion: 6, remoteUpdatedAt: DateTime(2025, 1, 1), remoteDeviceId: 'B',
      ), isTrue);

      // 2. Local higher version wins
      expect(SyncUtils.shouldRemoteOverwriteLocal(
        localVersion: 7, localUpdatedAt: DateTime(2026, 1, 1), localDeviceId: 'A',
        remoteVersion: 6, remoteUpdatedAt: DateTime(2027, 1, 1), remoteDeviceId: 'B',
      ), isFalse);

      // 3. Same version, remote newer timestamp wins
      expect(SyncUtils.shouldRemoteOverwriteLocal(
        localVersion: 10, localUpdatedAt: DateTime(2026, 1, 1, 10), localDeviceId: 'A',
        remoteVersion: 10, remoteUpdatedAt: DateTime(2026, 1, 1, 11), remoteDeviceId: 'B',
      ), isTrue);

      // 4. Same version, same timestamp, deviceId tie-breaker
      expect(SyncUtils.shouldRemoteOverwriteLocal(
        localVersion: 10, localUpdatedAt: DateTime(2026, 1, 1), localDeviceId: 'DeviceA',
        remoteVersion: 10, remoteUpdatedAt: DateTime(2026, 1, 1), remoteDeviceId: 'DeviceB',
    ...
    test('Resolution Pipeline: Local Win flow', () async {
      ...
      final s = Station(
        name: 'Initial', lat: 41.31, lng: 69.24, altitude: 450, 
        strike: 0, dip: 0, azimuth: 0, date: DateTime.now(), version: 1,
      );
      ...
      final localItem = syncBox.values.last; // Boyagi addStation dan tushgan item
      ...
      final resolvedStation = Station.fromMap(conflict.localData).copyWith(version: newLocalVersion);
      await stationRepository.updateStation(key, resolvedStation, customRequestId: reqId);

      // 4. Tekshiruv
      final finalLocal = stationBox.get(key)!;
      expect(finalLocal.version, 4); // v2(remote) + 1 = 3, keyin _stamp yana +1 qiladi = 4
      ...
      print('✅ SUCCESS: Resolution Pipeline yopildi (v1 -> Conflict(v2) -> Resolve v3)');
    });
    test('CHAOS TEST: 100 items + Random network/crash simulation', () async {
      // 1. 100 ta amalni navbatga qo'shish
      for (int i = 0; i < 100; i++) {
        final s = Station(
          name: 'Chaos Station $i', lat: 41.31, lng: 69.24, altitude: 450,
          strike: 0, dip: 0, azimuth: 0, date: DateTime.now(),
        );
        await stationRepository.addStation(s);
      }
      ...
      print('✅ SUCCESS: Chaos Test (Crash + Network toggle) muvaffaqiyatli o\'tdi. Data Integrity saqlandi.');
    });
  });
}
...
void _registerAdapters() {
  ...
  if (!Hive.isAdapterRegistered(11)) Hive.registerAdapter(SyncOperationAdapter());
  if (!Hive.isAdapterRegistered(12)) Hive.registerAdapter(SyncConflictAdapter());
...
class MockCollection extends Fake implements CollectionReference<Map<String, dynamic>> {
  @override
...
class MockSnapshot extends Fake implements DocumentSnapshot<Map<String, dynamic>> {
  @override
